...
public class Receiver extends HttpServlet {
    ...
    private static LinkedHashMap<String, ArrayList> taskCostsMap = new LinkedHashMap<>();
    private static LinkedHashMap<String, String> resultParamsMap = new LinkedHashMap<>();
    private static LinkedHashMap<String, NodeUser> nodeUsersMap = new LinkedHashMap<>();
    private static LinkedHashMap<String, CompletedRenderTask> completedTasksMap = new LinkedHashMap<>();
    ...
    final DbxRequestConfig config = DbxRequestConfig.newBuilder("dropbox/nebula-render").build();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        System.out.println("------------------- END (Receiver - doGet) -------------------");
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (finalResult != null) {
            handleFinalResult(finalResult, resultParamsMap.get("userEmail"), resultParamsMap.get("taskID"), resultParamsMap.get("taskFileName"));
        }

//        existingFrames.add(Integer.valueOf(resultParamsMap.get("renderFrame")));
//        InputStream fileContent = resultFilePart.getInputStream();

        // RESULT STORAGE DIRECTORY CREATION & INITIALIZATION
//        originalTaskFile = new File (taskDatabase + "/" + resultParamsMap.get("taskID"));
//        nodeResults = new File(taskDatabase + "/" + resultParamsMap.get("taskID") + "/noderesults");
//        clientCollection = new File(taskDatabase + "/" + resultParamsMap.get("taskID") + "/clientcollection");
//        frameResults = new File(nodeResults, frameID);
//        finalResults = new File(nodeResults, "/finalresults");
//        File finalResult = null;
//
//        if (!frameResults.exists()) {
//            frameResults.mkdir();
//            System.out.println(frameID + " directory created.");
//        }

        // STORING RESULT TO RESULT STORAGE DIRECTORY - TaskID > FrameID > SubtaskID :

            // RECORDING AND INITIALIZING A COMPLETED_SUBTASK OBJECT FOR REFERENCE & TRACKING :
//            CompletedRenderTask.CompletedSubtask completedSubtask = new CompletedRenderTask.CompletedSubtask(resultParamsMap.get("nodeEmail"),
//                    resultParamsMap.get("deviceID"),
//                    resultParamsMap.get("ipAddress"),
//                    resultParamsMap.get("taskID"),
//                    resultParamsMap.get("subtaskID"),
//                    resultParamsMap.get("application"),
//                    Integer.valueOf(resultParamsMap.get("subtaskCount")),
//                    new File(""),
//                    resultParamsMap.get("computeMinutes"),
//                    Double.parseDouble(resultParamsMap.get("cost")));
//
//            completedSubtask.getExistingFramesArrayList().add(Integer.valueOf(resultParamsMap.get("renderFrame")));
//
//            //  RECORDING NODE TO TRACK COMPLETED WORK AND PAYMENT OWED TO NODE :
//            System.out.println("ADDING " + resultParamsMap.get("nodeEmail") + " to Node User list. Node User Size : " + nodeUsers.size());
//            NodeUser nodeUser = new NodeUser(resultParamsMap.get("nodeEmail"));
//            addNodeUser(nodeUser, completedSubtask);

//            if (Integer.valueOf(resultParamsMap.get("subtaskCount")) == 1) {
//                Files.copy(fileContent, new File(finalResults, fileName).toPath(), StandardCopyOption.REPLACE_EXISTING);
//                List<File> nodeResultList = listOfResultFiles(nodeResults);
//                List<File> finalResultList = listOfResultFiles(finalResults);
//                subtaskCheck(nodeResultList);
//
//                if (finalResultsCheckpoint(listOfResultFiles(finalResults))) {
...
//                        }
//                        finalResult = zipFiles(resultParamsMap.get("taskID"), filesToZip, nodeResults);
////
...
//                }
//
//            } else if (Integer.valueOf(resultParamsMap.get("subtaskCount")) > 1) {
...
//                if (frameResultsCheckpoint(frameID, nodeResultList, frameResultList)) {
//
...
//
//                        List<File> finalResultList = listOfResultFiles(finalResults);
//                        if (finalResultsCheckpoint(listOfResultFiles(finalResults))) {                                                              // todo - edits made here for finalResults
//
...
//                // THIS SECTION CHECKS IF SUBTASKS ARE COMPLETED (EACH FRAME), AND THEN IF OVERALL TASK IS COMPLETED (ALL FRAMES)
    ...
    public File handleResult(Part resultFilePart, String application, String fileName) throws IOException {
        ...
        if (application.contains("blenderCycles")) {
            System.out.println("RECEIVER | CHECK 1(A)");
            String frameID = String.format("%s_%03d_%03d", resultParamsMap.get("taskID"), Integer.valueOf(resultParamsMap.get("frameCount")), Integer.valueOf(resultParamsMap.get("renderFrame")));
//          InputStream fileContent = resultFilePart.getInputStream();
            ...
            System.out.println("TASK APPLICATION DOESN'T EXIST");
        }
    ...
    private static LinkedHashMap<String, String> extractResultParams (String paramsString) {
        ...
        if (paramsString.contains("blender")) {
            ...
            map.put("taskFileName", params.get(13));
    ...
    public void updateQueue() {
        ...
        System.out.println(taskArray[0].getName() + " is being deleted from Task Queue.");
        taskArray[0].getAbsoluteFile().delete();
    }
    ...
    public static void postRenderToNebula(String userEmail,
                                          String originalTaskFileName,
                                          String taskId,
                                          String cost,
                                          String computeTime,
        ...
        try {
            ...
            request.addHeader("original-filename", originalTaskFileName);
...
//            for (int i = 0; i < nodeUserAllCompletedSubtasks.size(); i++) {
//                String subtaskID = nodeUserAllCompletedSubtasks.get(i).getSubtaskID();
//                double computeHours = Double.parseDouble(nodeUserAllCompletedSubtasks.get(i).getComputeMinutes());
//                double profit = nodeUserAllCompletedSubtasks.get(i).getCost();
//                totalProfit += profit;
//                totalComputeHours += computeHours;
//                String string = i + ". " + subtaskID + " | PROFIT : RM " + profit + " | Compute Hour(s) : " + computeHours + "\r\n";
//                System.out.println(string);
//                costList.append(string);
//            }
